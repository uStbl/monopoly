        ...
        public Deck(bool isChest)
        {
            if (isChest)
            {
                cards = new List<Card>
                {
                    ...
                    new Card("Go to jail", false,
                        player =>
                        {
                            player.MoveTo(containingGame.FindPosition("Jail"));
            ...
            else
            {
                ...
                {
                    new Card("Advance to GO", false, player => {player.MoveTo(containingGame.FindPosition("GO"));}),
                    new Card("Advance to Illinois Ave. - If you pass GO, collect $200", false, player => {player.MoveTo(containingGame.FindPosition("Illinois Avenue"));}),
                    new Card("Advance to St. Charles Place - If you pass GO, collect $200", false, player => {player.MoveTo(containingGame.FindPosition("St. Charles Place"));}),
                    ...
                    new Card("Take a walk on the Boardwalk – Advance token to Boardwalk", false, player => {player.MoveTo(containingGame.FindPosition("Boardwalk"));}),
                    new Card("Take a trip to Reading Railroad – If you pass GO, collect $200", false, player => {player.MoveTo(containingGame.FindPosition("Reading Railroad"));}),
